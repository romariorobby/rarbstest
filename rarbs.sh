...
#     esac
# }
# TODO: P1: change $pmpass to password Box
getuserpm(){
    case $ppick in
        "B")
...
passmgr_passunix(){
	...
	[[ -d "$PASSWORD_STORE_DIR" || -n `ls $PASSWORD_STORE_DIR` ]] && mv $PASSWORD_STORE_DIR $PASSWORD_STORE_DIR-bak && rm -rf $PASSWORD_STORE_DIR
...
decryptgpg(){
    ...
    case "$OS" in
        ...
        "Linux") 
            ...
            gpgdir="/home/$name/.local/share/vault"
            gpg /tmp/aqs.tar.gz.asc
